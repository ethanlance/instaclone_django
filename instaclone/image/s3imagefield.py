...
class S3Storage(FileSystemStorage):
    ...
    def url(self, name):
        return Key(self.bucket, name).generate_url(100000, force_http=True)
...
class S3EnabledFileField(models.FileField):
    def __init__(self, bucket=settings.AWS_TEMP_UPLOAD_BUCKET, verbose_name=None, name=None, upload_to='', storage=None, **kwargs):
...
class S3EnabledImageField(models.ImageField):
    def __init__(self, bucket=settings.AWS_TEMP_UPLOAD_BUCKET, verbose_name=None, name=None, width_field=None, height_field=None, **kwargs):
        if settings.USE_AMAZON_S3:
            self.connection = S3Connection(settings.AWS_ACCESS_KEY, settings.AWS_SECRET_KEY)
            if not self.connection.lookup(bucket):
                self.connection.create_bucket(bucket)
